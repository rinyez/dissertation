# Appendix B.
# Modified TK model from Verner et al. (2016)
# Translated by Jung Y. (University of California, Irvine)
# Modified by Jung Y. and Bartell S.M. 
...
# Gained weight (kg) in a pregnant woman (Lee et al. 2010)
# x=gestational age (year, by =0.1 year); y=median weight (kg)
table_preg_weight=list("years"=c(0,11.5,24,37.5)/52,"y"=(c(0.0,2.55,8.29,13.92)))
table_preg_weight = data.frame(approx(table_preg_weight$years,table_preg_weight$y, xout=round(seq(from=0,to=0.769,by=0.1),1), rule=2))
BW_preF_func <- approxfun(table_preg_weight$x, table_preg_weight$y, rule = 2) 
table_post_weight=list("years"=c(0,2.5,4.5,11.5,24.5,52)/52,"y"=(c(13.92-3.28,6.30,5.92,5.00,3.31,0)))
table_post_weight = data.frame(approx(table_post_weight$years,table_post_weight$y, xout=round(seq(from=0,to=1,by=0.1),1), rule=2))
BW_postF_func <- approxfun(table_post_weight$x, table_post_weight$y, rule = 2) 

# Proportion of calories intake changes during pregnancy
# x=gestational age (year, by =0.1 year)
# y=recommended calorie by fraction
table_Cal_preg = list("years"=c(0.0, 7, 20, 34)/52, "y"=c(1800,1800,2140,2250)/1800)
table_Cal_preg = data.frame(approx(table_Cal_preg$years,table_Cal_preg$y, xout=round(seq(from=0, to=0.769, by=0.1),1), rule=2))
diet_preg_func <- approxfun(table_Cal_preg$x, table_Cal_preg$y, rule=2)
...
# Modified TK
preds_modif <- data.frame(Age_M-Age_conception)
for (k in 1:4) {
  Compound = k                            # Compound (1:PFOS, 2:PFOA, 3:PFHxS, 4:PFNA)
  #******************************************************************************************************
  # Compound
  PFOS = 1*(Compound == 1)
  PFOA = 1*(Compound == 2)
  ...
  diet_edi = (PFOS_diet*PFOS + PFOA_diet*PFOA + PFNA_diet*PFNA + PFHxS_diet*PFHxS)/1000
  ...
  PFOS_PMilk = 0.0130    # PFOS
  PFOA_PMilk = 0.0733    # PFOA
  PFHxS_PMilk = 0.0133   # PFHxS
  PFNA_PMilk = 0.030     # PFNA
  PMilk = PFOS*PFOS_PMilk+PFOA*PFOA_PMilk+PFHxS*PFHxS_PMilk+PFNA*PFNA_PMilk
  
  # Cord:maternal plasma level ratio
  PFOS_P_CM = 0.422      # PFOS
  PFOA_P_CM = 0.766      # PFOA
  PFHxS_P_CM = 0.564     # PFHxS
  PFNA_P_CM = 0.502     # PFNA
  ...
  PFNA_HL = 3.52         # PFNA 
  # Half-lives (day)
  HALF_LIFE = (PFOS*PFOS_HL+PFOA*PFOA_HL+PFHxS*PFHxS_HL+PFNA*PFNA_HL)*365 
  ...
  pbpkmodel <- function(Time, State, Parmeters) {
    with(as.list(c(State, Paras)), {
      ...
      BW_M = (Weight_prepregnancy_M/BW_F_func(Age_conception))*BW_F_func(Time/365) +
        ((IO_CONCEPTION-IO_DELIVERY)*(BW_preF_func(Age_C_gest)-BW_C) +
           (IO_DELIVERY)*BW_postF_func(Age_C))
      #******************************************************************************************************
      ...
      INTAKE_M = 
        ...
        diet_edi*BW_M*diet_preg_func(Time/365-Age_conception) +  
  ...
  preds_modif = cbind(preds_modif, out1)
}
colnames(preds_modif) <- c("time", "PFOS","PFOA","PFHxS","PFNA")

preds_modif <- preds_modif %>% 
  filter(time > 0.769) %>% 
